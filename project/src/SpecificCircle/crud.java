...
public class crud {
	...
	public void makePost(String title, String contents) {
		...
		sql = "INSERT INTO BOARD VALUES(" + (countB + 1) + ",TO_DATE('" + getCurrentDate()
				+ "', 'yyyy/mm/dd HH24:MI'),'" + title + "','" + contents + "'," + tid + "," + Cid + ",'"
				+ user.getUserId() + "')";
	...
	public void updatePost(int flag, String content, int Tid, int Bid, User user) { // 게시글 수정 ,삭제
		...
		if (flag == 1) // 수정
		{
			sql = "UPDATE BOARD SET content='" + content + "' where user_id='" + user.getUserId() + "' and tid=" + Tid
					+ " AND cid=" + Cid + " AND id=" + Bid;
		...
		{
			sql = "DELETE FROM BOARD where user_id='" + user.getUserId() + "' and tid=" + Tid + " AND cid=" + Cid
					+ " AND id=" + Bid;
			System.out.println(sql);
			if (db.updateSql(sql) == -1) {
	...
	public String getCircleName() {
		...
		try {
			String sql = "";
			sql = "SELECT C.Cname " + "FROM CIRCLE C " + "WHERE C.id = " + String.valueOf(Cid);
	...
	public int getCid() {
		...
		try {
			String sql = "";
			sql = "SELECT C.id " + "FROM CIRCLE C " + "WHERE C.cname LIKE '%" + circleName + "%'";
	...
	public int getTid() {
		...
		try {
			...
			String sql = "";
			sql = "SELECT T.id " + "FROM TAB_MENU T " + "WHERE T.tname LIKE '%" + currentTab.tabname + "%' AND T.cid="
					+ Cid;
	...
	public int getBoardcount(int Tid) {
		...
		try {
			...
			String sql = "";
			sql = "SELECT count(*) " + "FROM BOARD B " + "WHERE B.tid=" + Tid + " AND B.cid=" + Cid;
	...
	public int getCommentcount(int Bid, int Tid) {
		...
		try {
			...
			String sql = "";
			sql = "SELECT count(*) " + "FROM COMMENTS C " + "WHERE C.tid=" + Tid + " AND C.cid=" + Cid + " AND C.bid="
					+ Bid;
	...
	public void makeComment(int Bid, int Cid, int Tid, User user, String comment) {
		...
		String sql = "";
		sql = "INSERT INTO COMMENTS VALUES(" + (countC + 1) + ",TO_DATE('" + getCurrentDate()
				+ "', 'yyyy/mm/dd HH24:MI'),'" + comment + "'," + Bid + "," + Tid + "," + Cid + ",'" + user.getUserId()
				+ "')";
		// System.out.println(sql);
		if (db.updateSql(sql) == -1) {
	...
	public void ShowPost(int tid, User user) {
		...
		try {
			String sql = "";
			sql = "SELECT id,title,content,bdate " + "FROM BOARD b " + "WHERE user_id LIKE '%" + user.getUserId()
					+ "%' AND tid=" + tid + "AND cid=" + Cid;
